class Solution {
    ...
    public boolean search(int[] nums, int target) {
        ...
        while (start <= end) {
            ...
            // if start , mid and end are equal, then can't find if it is sorted array by
            // start<=mid, so if all 3 are equals then we trim down start and end by one and
            // continue execution from start,
            // continue execution form start this is because second(start+1) mid( new mid ),
            // end(end-1) might also be equals to each other , unless we find all three are
            // not equal we continue to discard
